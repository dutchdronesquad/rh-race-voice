...
- `race_relay.py`: `RaceRelaySink` — a `PlaybackSink` forwarding a `CalloutPlan`'s audio (by sha256 content reference, with cache-miss upload recovery) and context to a remote relay over the `race-relay/1` wire contract, retrying a failed delivery up to three times. `deadline`/`target` cross the hop as the primary's own monotonic timestamps, translated through a real `ClockMapping` exchange (`POST /v2/relay/clock`); `push_context` (`POST /v2/relay/state`) tells the receiver which context is current, wired from `RaceIngest`'s generation-bump hook so stale audio stops there too.
- `race_relay_receiver.py`: `RaceRelayReceiver` — the other end of `race-relay/1`: accepts relayed audio+context and plays it through this host's own local output, bypassing `PreparationPlanner`/`SynthesisWorker` entirely since the audio already arrived synthesized. Rejects audio from a superseded context and deduplicates `origin_event_id` (32-entry bound). Opt-in (`--relay-receiver-enabled`), gated by the same `--relay-token` as the sending primary.
