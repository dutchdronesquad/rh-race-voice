...
The primary-to-relay asset API is `race-relay/1`: `POST /v2/relay/assets` announces content by sha256 and gets back which hashes are missing; `PUT /v2/relay/assets/{sha256}` uploads only the missing ones; `POST /v2/relay/events` forwards the callout by content reference (`audio_refs`), carrying `origin_event_id` for correlation and `deadline_wall`/`target_wall` (approximate wall-clock timestamps, not yet a bounded-uncertainty `ClockMapping`). `race_relay.py`'s `RaceRelaySink` sends this; `race_relay_receiver.py`'s `RaceRelayReceiver` (opt-in, its own bearer token) receives it and plays the audio through its own local output, skipping synthesis entirely. Still open, tracked under #301: a real hop clock exchange (wall-clock stays interim), propagating stop/heat invalidation to the remote, and sender-side retry (with the receiver-side dedup that would require).
