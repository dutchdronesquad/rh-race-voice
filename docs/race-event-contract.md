...
Use bounded HTTP JSON requests over a persistent connection for race events and full state snapshots. Separate the state/control sender from disposable audio events so a slow audio request cannot hold a stop or heat change hostage. Both operate on a single destination-owned state machine. No message broker is needed.

The primary-to-relay asset API is `race-relay/1`: `POST /v2/relay/assets` announces content by sha256 and gets back which hashes are missing; `PUT /v2/relay/assets/{sha256}` uploads only the missing ones; `POST /v2/relay/events` forwards the callout by content reference (`audio_refs`), carrying `origin_event_id` for correlation and `deadline`/`target` (the primary's own monotonic timestamps, translated by a real four-timestamp `ClockMapping` exchanged over `POST /v2/relay/clock`, the same mechanism used for the RH-to-primary pairing). `POST /v2/relay/state` propagates the primary's current context so the receiver rejects audio from a superseded generation/heat instead of playing it stale. A failed delivery retries up to three times before dropping; `origin_event_id` is deduplicated on arrival (32-entry bound, mirroring `CacheCommands`) so a retried duplicate is never played twice. `race_relay.py`'s `RaceRelaySink` sends all of this; `race_relay_receiver.py`'s `RaceRelayReceiver` (opt-in, its own bearer token) receives it and plays the audio through its own local output, skipping synthesis entirely. Still open, tracked under #301: encrypted transport beyond a bearer token over whatever the operator fronts it with, per-destination pilot selection, and per-destination status exposure.
