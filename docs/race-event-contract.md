...
The primary-to-relay asset API is `race-relay/1` (`sendspin_service/race/race_relay.py`, `RaceRelaySink`): `POST /v2/relay/assets` announces content by sha256 and gets back which hashes are missing; `PUT /v2/relay/assets/{sha256}` uploads only the missing ones; `POST /v2/relay/events` forwards the callout by content reference (`audio_refs`), carrying `origin_event_id` for correlation and `deadline_wall`/`target_wall` (approximate wall-clock timestamps, not yet a bounded-uncertainty `ClockMapping` -- there is no real receiver yet to exchange clocks with). This is a first increment: the receiving side, the hop clock exchange, and propagating stop/heat invalidation to the remote all remain open, tracked under #301.
